...
rsSub.Source = "SELECT *, (SELECT COUNT (*) FROM LINKS WHERE LINKS.SUB_ID = LINK_SUBS.SUB_ID) AS LINK_COUNT  FROM LINK_SUBS  WHERE CAT_ID = " + Replace(rsSub__MMColParam, "'", "''") + "  ORDER BY SUB_NAME ASC"
rsSub.CursorType = 0
...
rsFeat.Source = "SELECT *, (LINK_RATE/NO_RATES) AS RATING, (SELECT COUNT(*) FROM COMMENTS WHERE RESOURCE_ID = LINK_ID AND RESOURCE_TYPE = 'LINKS') AS COMMENTS  FROM LINKS  WHERE CAT_ID = " + Replace(rsFeat__MMColParam, "'", "''") + " AND LINK_APPROVED = Yes  ORDER BY LINK_RATE DESC"
rsFeat.CursorType = 0
...
rsLink.Source = "SELECT *, (LINK_RATE/NO_RATES) AS RATING, (SELECT COUNT(*) FROM COMMENTS WHERE RESOURCE_ID = LINK_ID AND RESOURCE_TYPE = 'LINKS') AS COMMENTS  FROM LINKS  WHERE CAT_ID = " + Replace(rsLink__MMColParam, "'", "''") + " AND LINK_APPROVED = Yes  ORDER BY LINK_NAME ASC"
rsLink.CursorType = 0
                    ...
                    <table width="100%" border="0" cellspacing="0" cellpadding="0">
                      <tr> 
                        <td align="left" valign="middle" height="20" colspan="2"> 
                          <div class = "links"><font face="Verdana, Arial, Helvetica, sans-serif" size="2"><b><font size="1">&nbsp;<a href="../default.asp">HOME</a> 
                            &gt; <a href="default.asp">DIRECTORY</a> &gt; <a href="dirCat.asp?<%= MM_keepNone & MM_joinChar(MM_keepNone) & "id=" & rsCat.Fields.Item("CAT_ID").Value %>"><%= UCase((rsCat.Fields.Item("CAT_NAME").Value)) %></a> (<%=(rsCat.Fields.Item("LINK_COUNT").Value)%>) :</font></b></font></div>
                        </td>
                      </tr>
                      <tr> 
...
%>
                              <td align="left"> 
                                <div class = "links"> 
                                  <li><font face="Verdana, Arial, Helvetica, sans-serif" size="2"><b><a href="dirSub.asp?catid=<%=(rsSub.Fields.Item("CAT_ID").Value)%>&subid=<%=(rsSub.Fields.Item("SUB_ID").Value)%>"><%=(rsSub.Fields.Item("SUB_NAME").Value)%></a></b> <i>(<%=(rsSub.Fields.Item("LINK_COUNT").Value)%>)</i></font></li>
                                </div>
                              </td>
                              <%
	startrw = startrw + 1
 ...
 %>
                      ...
                      <tr> 
                        <td align="left" valign="middle" bgcolor="#00CC99" class = "bg_navigator" colspan="2">&nbsp;<b><font face="Verdana, Arial, Helvetica, sans-serif" size="1">FEATURED 
                          LINKS</font></b></td>
                      </tr>
                      <tr> 
...
%>
                      ...
                      <tr> 
                        <td align="left" valign="middle" bgcolor="#00CC99" height="20" class = "bg_navigator">&nbsp;<b><font face="Verdana, Arial, Helvetica, sans-serif" size="1"><%= UCase((rsCat.Fields.Item("CAT_NAME").Value)) %> LINKS</font></b></td>
                        <td align="right" valign="middle" bgcolor="#00CC99" height="20" class = "bg_navigator"> 
